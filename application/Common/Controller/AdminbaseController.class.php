...
class AdminbaseController extends AppframeController {
    ...
    function setstatus(){
    	...
    	if(isset($_GET['id'])){
    		$id = intval(I("get.id"));
    		$status = $model->where(array($pk=>$id))->getField($type);
    		if($status==1){
    			$status=0;
    			$data['msg']= '<i class="fa fa-close fa-lg"></i>';
    		}else{
    			$status=1;
    			$data['msg']= '<i class="fa fa-check fa-lg"></i>';
    		}
    		$data[$type] = $status;
    		$ret = $model->where( $pk. '=' . $id )->save( $data );
    		$data['status'] = 1;
    		$this->ajaxReturn( $data );
    	}
    	if(isset($_POST['ids']) && isset($_GET['value'])){
    		$data[$type] = $value ;
    		$ids = join(",",$_POST['ids']);
    		if ( $model->where( $pk. " in ($ids)")->save($data)!==false) {
    			$this->success("设置成功！");
    		} else {
    			$this->error("设置失败！");
    		}
