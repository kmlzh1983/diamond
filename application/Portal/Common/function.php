...
function sp_sql_posts_paged( $tag ){
	$where=array();
	$tag=sp_param_lable($tag);
	$field = !empty($tag['field']) ? $tag['field'] : '*';
	$limit = !empty($tag['limit']) ? $tag['limit'] : '';
...
function getCatee(){//废弃啦.见getCate函数
	$cate = M('Cate')->where( array('status'=>1 , 'parent'=>0) )->order('listorder')->select();
	$str = $sub = '';
	$i = 0;
	$subMenu = C('SUB_MENU');
	foreach($cate as $v){
		$str .= "<div class='catebox'>
								<i class='hd-icon hd-icon-1'></i>
		...
		$i ++;
		$cateSub = M('Type t')->field('ta.*')->join("left join ".C('DB_PREFIX')."type_attr ta on t.id=ta.type_id")->where("t.name='{$v['name']}'")->select();
		//var_dump($cateSub);echo '<br>';echo '<br>';echo '<br>';echo '<br>';
		//$substr = '';
		$substr = '';
		$catesubb = M('Cate')->where( array('status'=>1 , 'parent'=>$v['id']) )->order('listorder')->select();
		$substrr = '';
		if(!in_array($v['id'] , array(7,8,9))){
			foreach($catesubb as $vv){
			...
			foreach($cateSub as $value){
				if(empty($cateSub[0]['name'])) continue;
				$values = explode("\n" , $value['values']);
				$dd = '';
			...
			foreach($catesubb as $vv){
				$catesubbb = M('Cate')->where( array('status'=>1 , 'parent'=>$vv['id']) )->order('listorder')->select();
				$dd = '';
				foreach($catesubbb as $vvv){
