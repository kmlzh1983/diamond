...
class UserController extends AdminbaseController{
	...
	function index(){
		$count=$this->users_obj->count();
		$page = $this->page($count, 20);
		$users = $this->users_obj->order("create_time DESC")
	...
	function userinfo_post(){
		if (IS_POST) {
			...
			->field("user_login,user_email,last_login_ip,last_login_time,create_time,user_activation_key,user_status,role_id,score",true)//排除相关字段
			->create();
			if ($create_result) {
        ...
        $id=intval($_GET['id']);
    	if ($id) {
    		$rst = $this->users_obj->where(array("id"=>$id))->setField('user_status','0');
    		if ($rst) {
    ...
    function cancelban(){
    	$id=intval($_GET['id']);
    	if ($id) {
    		$rst = $this->users_obj->where(array("id"=>$id))->setField('user_status','1');
    		if ($rst) {
