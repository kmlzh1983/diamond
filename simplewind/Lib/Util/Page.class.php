    ...
    public function Pager($Page_tpl = '') {
        ...
        if ($cfg ['pageindex'] > 1) {
            ...
            if ($this->Static && ($cfg ['pageindex'] - 1) == 1) {
                $pPrev = $this->linkwraper_pre.'<a href="' . $this->PageLink['index'] . '" class=" link ajax ">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after; //上一页
            } else {
                $pPrev = $this->linkwraper_pre.'<a href="' . str_replace('*', $cfg ['pageindex'] - 1, $cfg ['link']) . '"  class=" link ajax ">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after;
            }
        } else {
        	$pPrev = $this->linkwraper_pre.'<a href="javascript:void(0)"  class="link">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after; //上一页
        }
        //下一页，尾页
        if ($cfg ['pageindex'] < $cfg ['pagecount']) {
            ...
            $pNext = $this->linkwraper_pre.'<a href="' . str_replace('*', $cfg ['pageindex'] + 1, $cfg ['link']) . '"  class=" link ajax ">' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
        } else {
        	$pNext = $this->linkwraper_pre.'<a href="javascript:void(0)"  class="link">' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
        }

        //快捷跳转方式
        switch (strtolower($cfg ['jump'])) {
            case 'input' :
                $pJumpValue = "$('#pagenum').val()";
                ...
                $pJump .= '}return false;}" />';*/
                $pJump .= '<div class="form">';
                $pJump .= '<span class="txt">到第</span>';
                $pJump .= ' <input class="input" type="text" name="pagenum" id="pagenum" value="" />';
                $pJump .= '<span class="txt">页</span>';
                $pJump .= '<span class="btn btn-sm submit" onclick="document.location.href=\'' . str_replace('*', '\'+' . $pJumpValue . '+\'', $cfg ['link']) . '\'">确定</span>';
                $pJump .= '</div>';
