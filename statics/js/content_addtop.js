...
authkey 参数密钥，验证args
**/
function flashupload(uploadid, name, textareaid, funcName, args, module, catid, authkey,file_upload_limit) {
    var args = args ? '&args=' + args : '';
    var setting = '&module=' + module + '&catid=' + catid ;
    Wind.use("artDialog","iframeTools",function(){
        art.dialog.open(GV.DIMAUB+'index.php?a=swfupload&m=asset&g=asset' + args + setting + '&file_upload_limit=' + file_upload_limit + '&textareaid=' + textareaid , {
        title: name,
        id: uploadid,
        width: '650px',
        height: '420px',
...
function change_images(uploadid, returnid) {
    ...
    $.each(contents, function(i, n) {
        ...
        var strTmp = '';
        if( 'goods_photos' == returnid )
        	strTmp = "&nbsp;&nbsp;排序:<input type='text' class='input-small' value='0' name='" + returnid + "_listorder[]'>&nbsp;&nbsp;<input type='radio' name='" + returnid + "_isIndex' class='input' value='randid_" + ids +"'>设为封面<input type='hidden' value='randid_" + ids +"' name='gid[]'>";
        str += "<li id='image" + ids + "'><input title='双击查看' type='text' name='" + returnid + "_url[]' value='" + n + "' style='width:310px;' ondblclick='image_priview(this.value);' class='input'> <input type='text' name='" + returnid + "_alt[]' value='" + filename + "' style='width:160px;' class='input' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\">" + strTmp +"&nbsp;&nbsp;<a href=\"javascript:remove_div('image" + ids + "')\">移除</a> </li>";
    });
