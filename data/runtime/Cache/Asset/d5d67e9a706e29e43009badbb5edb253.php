...
.eidt_uphoto .no {
    background: url("/diamond/tpl_admin/simpleboot/assets/images/upload_pic.jpg") no-repeat scroll center center #FBFBFB;
    border: 1px solid #CCCCCC;
    height: 98px;
    overflow: hidden;
    text-indent: -2000em;
...
.eidt_uphoto .del{
	...
	width:15px;
	height:15px;
	background:url("/diamond/tpl_admin/simpleboot/assets/images/upload_del.png") no-repeat;
	right:1px;
	top:1px;
	overflow:hidden;
	text-indent:-2000em;
...
/*上传选择按钮*/
#btupload,.addnew{
	background: url("/diamond/statics/js/swfupload/images/swfBnt.png") no-repeat; float:left; margin-right:10px;width:75px; height:28px; line-height:28px;font-weight:700; color:#fff;
}
#btupload{ 
...
var swfu = '';
	$(document).ready(function(){
		Wind.use("swfupload",GV.DIMAUB+"statics/js/swfupload/handlers.js",function(){
		      swfu = new SWFUpload({
			flash_url:"/diamond/statics/js/swfupload/swfupload.swf",
			...
			<?php if($file_upload_limit != 0): ?>file_upload_limit:"<?php echo ($file_upload_limit); ?>",<?php endif; ?>
    ...
    <div class="pop_cont">
      <div class="">
        <div class="edit_uping"> 
            ...
            <span class="num">
     		<?php if($file_upload_limit != 0): ?>最多上传<em> <?php echo ($file_upload_limit); ?></em> 个附件,<?php endif; ?>       
                     单文件最大 <em>20240 KB</em>，<em style="cursor: help;" title="可上传格式：jpg,jpeg,gif,png,bmp">支持格式？</em></span>
        </div>
